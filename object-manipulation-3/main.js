/*
Purpose: create a game that will decide the winner amongst letious players
...
make new let players and assign an empty array
make new let cards with all available cards  in the deck
...
once the loops are done, declare a new letiable for the winner array so scores can be stored
...
function runGame(array, handSize) {
  ...
  for (let i = 0; i < players.length; i++) {
    ...
    for (let f = 0; f < players[i].hand.length; f++) {
      const playerHand = players[i].hand[f].rank;
      if (playerHand === 'ace') {
        players[i].count += 11;
  ...
  const winner = _.orderBy(players, ['count'], ['desc']);
